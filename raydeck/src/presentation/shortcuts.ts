export function ignoresPresentationShortcut(event: KeyboardEvent) {
  ...
  if (target?.closest('input, textarea, select, [contenteditable="true"], [role="menu"], [role="dialog"], [role="separator"], .notes-zoom, .save-menu, .present-menu, .slide-picker')) return true;
